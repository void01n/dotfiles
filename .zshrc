...
fish_prompt() {
    local dir="${PWD/#$HOME/~}"
    local text="$USER@nixos: $dir/"
    local raw
    raw=$(printf "%s" "$text" | python3 ~/.config/shell/catppuccinize.py)
    local first_color
    first_color=$(printf "%s" "$raw" | grep -oP "\x1b\[38;2;[\d;]+m" | head -n1)
    local safe_raw
    safe_raw=$(printf '%s' "$raw" | sed -E $'s/(\x1b\[[0-9;]*m)/%{\\1%}/g')
    local safe_first_color="%{${first_color}%}"
    PROMPT="${safe_raw}"$'\n'"${safe_first_color}>%{"$'\e[0m'"%} "
}
add-zsh-hook precmd fish_prompt
...
unset SSH_ASKPASS
export PYTHONPATH="/nix/store/avs8b6nifyc92gwcic4wv2bcrfkwl6lb-python3.14-evdev-1.9.3/lib/python3.14/site-packages:$PYTHONPATH"
export NIXPKGS_ALLOW_UNFREE=1
