    ...
    (pkgs.symlinkJoin {
      ...
      postBuild = ''
        makeWrapper ${pythonEnv}/bin/python3 $out/bin/v0wwa \
          ...
          --run 'export GI_TYPELIB_PATH="$(c=$HOME/.cache/v0wwa_pangocairo_dir; [ -f "$c" ] && cat "$c" || { d=$(dirname "$(find /nix/store -maxdepth 5 -name PangoCairo-1.0.typelib 2>/dev/null | head -1)"); mkdir -p "$(dirname "$c")"; echo "$d" > "$c"; echo "$d"; }):$GI_TYPELIB_PATH"' \
